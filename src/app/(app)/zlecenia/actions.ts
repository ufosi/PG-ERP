...
const orderSchema = z.object({
  ...
  productionComments: z.string().trim().max(2000).optional(),
  officeComments: z.string().trim().max(2000).optional(),
...
export async function createProductionOrder(formData: FormData) {
  ...
  const parsed = orderSchema.parse({
    ...
    productionComments: formData.get("productionComments") || undefined,
    officeComments: formData.get("officeComments") || undefined,
  });
  ...
  await prisma.productionOrder.create({
    data: {
      ...
      productionComments: parsed.productionComments,
      officeComments: parsed.officeComments,
...
export async function updateProductionOrder(formData: FormData) {
  ...
  await prisma.productionOrder.update({
    ...
    data: {
      ...
      productionComments: parsed.productionComments ?? null,
      officeComments: parsed.officeComments ?? null,
      dueDate: parsed.dueDate ? new Date(parsed.dueDate) : null,
